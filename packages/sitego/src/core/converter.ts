...
export function htmlToMarkdown(html: string, userOptions?: Partial<typeof DEFAULT_OPTIONS>) {
  if (!html) return ''
...
function processNode(
  ...
  return parentNode.childNodes.reduce((output, child) => {
    const node = augmentNode(child, options)
    if (node.nodeType === 3) {
      return joinOutput(output, node.isCode ? node.data : escapeMarkdown(node.data))
    }
    return joinOutput(output, replacementForElement(node, options, references))
  }, '')
}
...
function joinOutput(output: string, replacement: string) {
...
function postProcess(output: string, options: typeof DEFAULT_OPTIONS, references: string[]) {
